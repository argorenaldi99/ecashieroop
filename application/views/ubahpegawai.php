<html>
	...
	<body>
		...
		<form id="inputpegawai" action="<?= base_url(); ?>admin/ubahpegawaiproses/<?= $row['id']; ?>" method="post">
			
			<div class="forminput">
			  	<div class="form-group">
			   		<label>Nama Pegawai</label>
			   		<input type="text" class="form-control col-lg-4" id="nama_pegawai" name="nama_pegawai" maxlength="30" onkeypress="return isNotNumberKey(event)" pattern=".{4,30}" required title="4 to 30 characters" value="<?= $row['nama_pegawai']; ?>">
			  	</div>
			  	<div class="form-group">
			   		<label>Jenis Kelamin</label>
			   		<select id="jeniskelamin" name="jeniskelamin">
					 	<option <?php if ($row['jeniskelamin'] == 'Laku-Laki') { echo 'SELECTED'; }; ?>>Laki-Laki</option>
					 	<option <?php if ($row['jeniskelamin'] == 'Perempuan') { echo 'SELECTED'; }; ?>>Perempuan</option>
					</select>
			  	</div>
			  	<div class="form-group">
			   		<label>Tanggal Lahir</label>
			   		<input type="date" class="form-control col-lg-4" id="tgllahir" name="tgllahir" min="1980-01-01" max="2001-12-31" value="<?= $row['tgllahir']; ?>" required>
			  	</div>
			  	<div class="form-group">
			   		<label>Alamat</label>
			   		<input type="text" class="form-control col-lg-4" id="alamat" name="alamat" value="<?= $row['alamat']; ?>" maxlength="30" pattern=".{4,30}" required title="4 to 30 characters">
			  	</div>
			  	<div class="form-group">
			   		<label>No Telepon</label>
			   		<input type="text" class="form-control col-lg-4" id="notlp" name="notlp" maxlength="12" onkeypress="return isNumberKey(event)" value="<?= $row['notlp']; ?>" pattern=".{10,12}" required title="10 to 12 characters">
			  	</div>
			  	<div class="form-group">
			   		<label>Jabatan</label>
			   		<input type="text" class="form-control col-lg-4" id="jabatan" name="jabatan" maxlength="20" onkeypress="return isNotNumberKey(event)" value="<?= $row['jabatan']; ?>" pattern=".{4,20}" required title="4 to 20 characters">
			  	</div>
			</div>
