	...
	<body>
		  		...
		  		<table class="table">
				    ...
				    <tbody>
	                    ...
	                    <td><select id="statushadir" name="statushadir">
	                    	<option <?php if ($d['statushadir'] == 'Hadir') { echo "SELECTED"; }; ?>>Hadir</option>
	                    	<option <?php if ($d['statushadir'] == 'Tidak Hadir') { echo "SELECTED"; }; ?>>Tidak Hadir</option>
	                    </select></td>
	                    </tr>
	                    <?php endforeach ?>
				    </tbody>
		...
		<script type="text/javascript">
			$(document).ready(function(){
				...
				if (confirm('Anda telah melakukan absensi hari ini, Apakah anda ingin mengubah data abasensi?')) {
			...
			$("#selesai").click( function(){
				...
				$.ajax({
	                method:"POST",
	                url:"<?= base_url(); ?>admin/ubahkehadiran",
	                data:{nama:nama, hadir:hadir, tanggal:tanggal},
	                success:function(result){
	                    if (result == "berhasil") {
	                    	alert("Berhasil melakukan update kehadiran");
	                    	window.location='<?= base_url(); ?>admin/kehadiran';
	                    } else {
	                    	alert(result);
	                    }
