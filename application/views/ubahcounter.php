<html >
	...
	<body>
		...
		<div >
		<form id="inputsewa" action="<?= base_url(); ?>admin/ubahcounterproses/<?= $no_counter_lama; ?>" method="post">
			<div class="forminput">
			  	<div class="form-group">
			   		<label>No Counter</label>
			   		<select id="no_counter" name="no_counter">
					 	<?php for ($i=1; $i <= 15; $i++) { 
					 		if (!in_array($i, $no_counter)) {  
					 			if ($i == $no_counter_lama) {?>
			  	...
			  	<div class="form-group">
			   		...
			   		<input type="text" class="form-control col-lg-4" id="namapenyewa" name="namapenyewa" maxlength="30" onkeypress="return isNotNumberKey(event)" pattern=".{4,30}" required title="4 to 30 characters" value="<?= $row['nama_penyewa']; ?>">
			  	</div>
			  	<div class="form-group">
			   		<label>Alamat</label>
			   		<input type="text" class="form-control col-lg-4" id="alamat" name="alamat" maxlength="30" pattern=".{4,30}" required title="4 to 30 characters" value="<?= $row['alamat']; ?>">
			  	</div>
			  	<div class="form-group">
			   		<label>No Telepon</label>
			   		<input type="text" class="form-control col-lg-4" maxlength="12" id="notlp" name="notlp" maxlength="12"  onkeypress="return isNumberKey(event)" pattern=".{10,12}" required title="10 to 12 characters" value="<?= $row['notlp']; ?>">
			  	</div>
			  	<div class="form-group">
			   		<label>Tanggal Awal Sewa</label>
			   		<input type="date" class="form-control col-lg-4" id="tglawalsewa" name="tglawalsewa" min="<?= $row['tglawalsewa']; ?>" onchange="settglakhir()" value="<?= $row['tglawalsewa']; ?>" required>
			  	</div>
			  	<div class="form-group">
			  	...
			  	<div class="form-group">
			   		...
			   		<input type="date" class="form-control col-lg-4" id="tglakhirsewa" name="tglakhirsewa" value="<?= $row['tglakhirsewa']; ?>" readonly>
			  	</div>
			  	<div class="form-group">
			   		<label>Biaya</label>
			   		<input type="number" class="form-control col-lg-4" id="biaya" name="biaya" value="<?= $row['biaya']; ?>" readonly>
			  	</div>
			  	<div class="form-group">
			   		<label>Bayar</label>
			   		<input type="number" class="form-control col-lg-4" min="100000" max="???" id="bayar" name="bayar" value="<?= $row['bayar']; ?>"  onkeypress="return isNumberKey(event)" required>
			  	</div>
			</div>
		...
		<script type="text/javascript">
			...
			$(document).ready(function(){
				...
				dt1 = new Date($("#tglawalsewa").val());
			    dt2 = new Date($("#tglakhirsewa").val());
			    var diff =(dt2.getTime() - dt1.getTime()) / 1000;
				   diff /= (60 * 60 * 24 * 7 * 4);
				  period = Math.abs(Math.round(diff));

				biayaawal = period*1000000;
				$('#bulan').val(period);
				$('#biaya').val(biayaawal);
